defmodule KinoK8s.WatchCell do
  use Kino.JS, assets_path: "lib/assets/watch_cell"
  use Kino.JS.Live
  use Kino.SmartCell, name: "K8s - Watch Resources"
  ...
  def init(attrs, ctx) do
    ctx =
      assign(ctx,
        ...
        connections: [],
        connection: nil,
        result_variable: Kino.SmartCell.prefixed_var_name("stream", attrs["result_variable"]),
        gvk: attrs[:gvk],
  ...
  def to_source(attrs) do
    if all_fields_filled?(attrs, [:connection, :result_variable, :gvk]) do
      %{
